...
fields = ['ticker', 'price', 'weight', 'old_value', 'new_value', 'old_shares', 'trade_quantity', 'new_shares', 'action', 'action_type', 'target_tag', 'execution_timestamp', 'custom_tag']
table_name ="recommendations"
primary_keys = ['ticker', 'target_tag','execution_timestamp']
...
def write_target_execution_2db(df, custom_tag = "None"):
...
def write_activity_2db(days=1):
    ...
    with pyodbc.connect(config.AZURE_CONNECTION_STRING) as cnxn:
        ...
        after_date = (now_date - timedelta(days=days)).isoformat(timespec='seconds')+"Z"
        activities = api.get_activities(after=after_date)
        print(f"Total Activities over {days} day(s):", len(activities))
        for activity in activities: 
            # print(activity.id, activity.order_id,activity.transaction_time,activity.symbol, activity.side, 
            # activity.qty,activity.leaves_qty,activity.cum_qty, activity.order_status, activity.type)
